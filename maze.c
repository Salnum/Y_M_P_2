...
void generate_adachi_shortestRoute(void){
	...
	while( !( x_coordinate == GOAL_X_COODINATE && y_coordinate == GOAL_Y_COODINATE ) ){
		...
		if( nextdir == 0 ){
			if( previousdir != nextdir )	i++;	//ひとつ前と今回の向きが同じでなければパス要素を進める
			path[i] ++;				//直進パス
			update_coordinate();			//座標更新
		}else if( nextdir == 1 ){
			i++;					//直進でないのでパス進める
			path[i] = 20;				//右折パス
			if(++m_dir > 3)	m_dir = 0;		//現在向き右へ
		}else if( nextdir == 3 ){
			i++;					//直進でないのでパス進める
			path[i] = 30;				//左折パス
			if(--m_dir < 0)	m_dir = 3;		//現在向き左へ
		}
	}
	path_size = i;
}
...
void add_wall(char _x_coordinate, char _y_coordinate, char _dir){
	...
	if(_dir == NORTH){
		...
		else if(_x_coordinate == 1 && _y_coordinate == 0)	return;	//これはスタート区画の東壁
